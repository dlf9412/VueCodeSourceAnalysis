...
export function optimize (root: ?ASTElement, options: CompilerOptions) {
  ...
  // 遍历所有节点，给每个节点设置static 属性，标识其是否为静态节点
  markStatic(root)
...
/**
 * 在所有节点上设置 static 属性，用来表示是否为静态节点
 * 注意： 如果有子节点为动态节点，则父节点也被认为是动态节点
 * @param {*} node 
 * @returns 
 */
function markStatic (node: ASTNode) {
  // 根据节点类型，判断是否为静态节点，为节点标识
  ...
  if (node.type === 1) {
    ...
    // 遍历子节点，递归调用markStatic 来标记这些子节点的static 属性
    ...
    // 如果节点存在 v-if,v-else-if,v-else 这些指令，则依次标记block中节点的static
...
/**
 * 判断节点是否为静态节点
 * 通过自定义的node.type 来判断：2：表达式=>动态，3：文本=>静态
 * 凡是有 v-bind,v-if,v-for 等指令的都属于动态节点
 * 组件为动态节点
 * 父节点为含有v-for 指令的template 标签，则为动态节点
 * @param {*} node 
 * @returns 
 */
