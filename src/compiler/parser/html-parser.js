...
export function parseHTML (html, options) {
  ...
  while (html) {
    ...
    // 到这里就处理结束，如果stack 数组中还有内容，则说明有标签没有被闭合，给出提示信息
    if (html === last) {
   ...
   * index 为html 在原始的模板字符串中 的开始索引，也是下一次该处理的字符的开始位置
   * @param {*} n 
   */
  ...
  function parseStartTag () {
    ...
    if (start) {
      ...
      let end, attr
      // 处理开始标签内的 各个属性，并将这些属性放到 match.attrs 数组中
   ...
   * 处理属性 match.attrs,如果不是自闭合标签，则将标签信息放到stack 数组，待将来处理到它的闭合标签时再将其弹出stack，表示该标签处理完毕，这是标签的所有信息都在element ast 对象上了
   * 接下来调用options.start 方法处理标签，并根据标签信息生成element ast
   * 以及处理开始标签的属性和指令，最后将element ast 放入stack 数组
   * @param {*} match 
   */
  function handleStartTag (match) {
    ...
    // 处理 match，attrs，得到attrs=[{name:attrName,value:attrVals,start:xx,end:xx},...]
    ...
    // 如果不是自闭合标签，则将标签信息放到stack 数组中，待将来处理到它的闭合标签时，再将其弹出stack
    // 如果时自闭合标签，则标签信息就没必要进入 stack 了，直接处理众多属性，将他们都设置到element ast 对象上，都没有
    if (!unary) {
      // 将标签信息放入 stack 数组中，{tag,lowerCasedTag,attrs,start,end}
      stack.push({ tag: tagName, lowerCasedTag: tagName.toLowerCase(), attrs: attrs, start: match.start, end: match.end })
      // 标识 当前标签的结束标签为tagName
      lastTag = tagName
    }

    /**
     * 调用 start 方法，主要做了一下 6件事情
     * 1、创建AST 对象
     * 2、处理存在 v-model 指令 的 input 标签，分别处理 input 为checkbox，redio，其他情况
     * 3、处理标签上的众多指令，比如v-pre,v-for,v-if,v-once
     * 4、如果根节点 root 不存在则设置当前元素为根节点
     * 5、如果当前元素为非自闭合标签则  将自己push 到stack 数组，并记录currentParent，在接下来处理子元素时用来告诉子元素
   ...
   * 解析结束标签，比如</div>
   * 最主要的事就是：
   * 1、处理完结束标签之后调整stack 数组，保证在正常情况下 stack 数组中的最后一个元素 就是 下一个结束标签对应的开始标签
   * 2、处理完结束标签之后调整stack数组，保证在正常情况下 stack 数组 中的最后一个元素就是下一个结束标签对应的开始标签
  ...
  function parseEndTag (tagName, start, end) {
    ...
    // 倒序遍历stack 数组，找到第一个和当前结束标签相同的标签，该标签就是结束标签对应的开始标签的描述对象
    ...
    // 如果在stack 中一直没有找到相同的标签名，则pos 就会<0,进行else 分支
    if (pos >= 0) {
      // 这个 for 循环 负责关闭 stack 数组索引 >= pos 的所有标签
      // 为什么要用一个循环，上面说到正常情况下 stack 数组的最后一个元素就是我们要做的开始标签
      // 但是有些异常情况，就是有些元素没有给提供结束标签，比如：
      // stack=['span','div','span','h1'],当前处理的结束标签 tagName = div
      // 匹配到 div,pos = 1,那索引 为2 和 3 的标签（span，h1）说明就没提供结束标签
      // 这个 for 循环就负责关闭 div 、span 和 h1这三个标签
      // 并在开发环境为 span 和 h1这两个标签给出 “未匹配到结束标签的提示”
